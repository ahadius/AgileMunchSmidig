	...
	useEffect(() => {
		...
		const draw = e => {
			...
			if (brushType === 'round') {
				...
				context.lineWidth =
					size * (Math.round(Math.random()) ? 1 : 0.5);
			}
		...
		const startDrawing = e => {
			// funksjoner for å bevege både musset og touch bevegelser
			e.preventDefault();
			const rect = canvas.getBoundingClientRect();
			const x =
				('clientX' in e
		...
		const endDrawing = e => {
			...
			console.log('Sending drawing data to server', {
				fromX: startPos.x,
				fromY: startPos.y,
				toX: endPos.x,
				toY: endPos.y,
